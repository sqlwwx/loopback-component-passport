          ...
          if (link) {
            if (req.user) {
              self.userCredentialModel.link(
                ...
                {token: token, tokenSecret: tokenSecret}, options, self.userIdentityModel, done);
            } else {
        ...
        function(req, identifier, profile, done) {
          if (link) {
            if (req.user) {
              self.userCredentialModel.link(
                req.user.id, provider, authScheme, profile,
                {identifier: identifier}, options, self.userIdentityModel, done);
            } else {
              done(g.f('No user is logged in'));
            }
          } else {
          ...
          if (link) {
            if (req.user) {
              self.userCredentialModel.link(
                ...
                }, options, self.userIdentityModel, done);
            } else {
              done(g.f('No user is logged in'));
            }
          } else {
          ...
          if (link) {
            if (req.user) {
              self.userCredentialModel.link(req.user.id, name, authScheme,
                profile, {}, options, self.userIdentityModel, done);
            } else {
