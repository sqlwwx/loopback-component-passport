...
module.exports = function(UserIdentity) {
  ...
  // Let's create a user for that
    ...
    // todo: 处理中文名
    var generatedEmail = (profile.mobilePhone || profile.username || profile.id) + '@loopback.' +
      (profile.provider || provider) + '.com';
    var email = provider === 'ldap' ? profileEmail : generatedEmail;
    var username = (options.notAppendProvider ? '' : (provider + '.')) +
      (profile.username || profile.id);
    var password = profile.password || utils.generateKey('password');
    var userObj = {
      username: username,
      password: password,
    };
    ...
    if (profile.mobilePhone) {
      userObj.mobilePhone = profile.mobilePhone;
    }
    return userObj;
  }
  ...
  UserIdentity.login = function(provider, authScheme, profile, credentials,
    ...
    profile.id = profile.id || profile.unionid || profile.openid;
        ...
        userIdentityModel.findOrCreate({where: {externalId: profile.id}}, {
          ...
          if (!err && user && autoLogin) {
            if (!identity.userId) {
              ...
              identity.updateAttributes({userId: user.id});
            }
            return (options.createAccessToken || createAccessToken)(user, function(err, token) {
              cb(err, user, identity, token);
            });
